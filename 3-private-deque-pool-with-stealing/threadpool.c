...
void *worker_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (!has_work)
        {
            has_work = steal_task(global_pool, my_id, &current_task);
        }
...
/**
 * @brief thread goes through other worker's queue randomly and steals work
 *
 * @param pool : pool of all threads
 * @param my_id : self explanatory
 * @param out_task : output queue to append the stolen task to
 *
 * @returns
 *  true: if work is stolen successfully
 *  false, otherwise
 *
 */
bool steal_task(thread_pool_t *pool, int my_id, task_t *out_task)
{
    int attempts = pool->num_workers;

    for (int i = 0; i < attempts; i++)
    {
        // we follow picking a random victim
        int victim_id = rand() % pool->num_workers;
        if (victim_id == my_id)
        {
            continue;
        }

        worker_queue_t *victim_queue = &pool->queues[victim_id];

        pthread_mutex_lock(&victim_queue->lock);
